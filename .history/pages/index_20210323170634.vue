<template>
  <div>
    ...
    <div class="container"> 
      ...
      <h2 id="tools" class="my-3 font-bold text-4xl text-white mt-10">Comment ?</h2>
      ...
      <h2 id="stages" class="my-3 font-bold text-4xl text-white mt-10">Stages et ateliers</h2>
      <div class="grid grid-flow-row lg:grid-flow-col gap-3 mt-5"
        id="stages">
        <div v-for="post in posts[10]" :key="post.id">
          <div class="flex items-center shadow rounded overflow-hidden bg-gray-100 mt-2">
            <div class="w-1/3">
              <img :src="post['_embedded']['wp:featuredmedia'][0]['media_details']['file']" :alt="post.title.rendered"
                v-if="post['_embedded']['wp:featuredmedia'] && post['_embedded']['wp:featuredmedia'].length > 0 && post['_embedded']['wp:featuredmedia'][0]['media_details']['file']"/>
            </div>
            <div class="w-2/3 lg:p-5">
              <span v-html="post.content.rendered"></span>
            </div>
